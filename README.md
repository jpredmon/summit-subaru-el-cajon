...
- **`integration_test` (Task 39):** `flutter test integration_test/<file>.dart -d <device-id>` runs a real, compiled build against a connected device or browser — closes a gap plain `flutter_test` can't (real network/image timing, real scroll physics). Targets the physical Pixel 2 by device id, not an emulator or `-d chrome` (avoids the RAM cost of a local browser on this machine). Two flows exist: `integration_test/srp_large_inventory_test.dart` (scroll a large inventory, apply/clear filters) and `integration_test/srp_error_retry_test.dart` (fetch failure → Retry → loaded), both verified on the real Pixel 2. One gotcha specific to real devices: `SkeletonPulse`'s loading animation repeats forever absent a reduced-motion setting, so `pumpAndSettle()` right after mounting a screen that starts in a loading state hangs until its 10-minute timeout — use a bounded/condition-based pump instead (`integration_test/support/pump_until.dart`). A locked device screen also stalls any `pump()` indefinitely (no VSYNC delivered); `adb shell svc power stayon usb` keeps it awake while connected.
